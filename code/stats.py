...
class StatsDF:
    ...
    def split_popularity(self, pf_fo, pf_fr, pc_fo, pc_fr):
        ...
        df['isPopular'] = False 
        df.loc[ ((df['user_followers'] >= fos_f)& (df['user_followers'] < fos_c)) & ((df['user_friends'] >= frs_f) & (df['user_friends'] < frs_c)), 'isPopular'] = True
    ...
    def country_analysis(self, sample_cutoff):
        ...
        for country in countries:
            tox_slice = temp_df[temp_df['clean_loc'] == country]['tox_binned'].map(consts.TOX_MAP)
            sent_slice = temp_df[temp_df['clean_loc'] == country]['sent_binned'].map(consts.SENT_MAP).astype(int)
            country_slice = [country] * len(sent_slice)
            df_slice = pd.DataFrame({'Country':country_slice, 'Toxicity': tox_slice, 'Sentiment': sent_slice})
